...
from clinical_ner.models.hf_extractor import (
    HFDecoderSpanExtractor,
    HFEncoderSpanExtractor,
)
...
def make_dataset_wise_config(config):
    ...
    for dataset in leaderboard_datasets:
        dataset_config = {
            "label_normalization_map": inference_config["label_normalization_map"][
                dataset
            ],
        ...
        if config["model_architecture"] == "Decoder":
            dataset_config = {
                **dataset_config,
                "prompt_template_identifier": inference_config[
                    "prompt_template_identifier"
                ],
            }
...
def reproduce_leaderboard_results(
    models: list | None = None, output_dir: str | None = None
):
    """
